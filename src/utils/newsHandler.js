...
window.copyToClipboard = (text) => {
  navigator.clipboard
    .writeText(text)
    .then(() => {
      alert("SYSTEM: Link berhasil disalin ke clipboard.");
    })
    .catch((err) => {
...
const createNewsCard = (item) => `
    <article class="group bg-slate-800/10 border border-slate-800/40 rounded-[2.5rem] overflow-hidden hover:border-sky-500/30 transition-all duration-500 flex flex-col h-full cursor-pointer" onclick="openNewsDetail('${item.id}')">
        <div class="relative h-56 overflow-hidden bg-slate-900">
            <img 
                src="${item.image}" 
                onerror="this.onerror=null; this.src='https://images.unsplash.com/photo-1555066931-4365d14bab8c?q=80&w=1000';"
                loading="lazy"
...
export const renderNewsGrid = () => {
  ...
  const btnClose = document.getElementById("btnCloseMore");

  if (btnLoad)
    btnLoad.classList.toggle("hidden", visibleCount >= filtered.length);
  if (btnClose) btnClose.classList.toggle("hidden", visibleCount <= 3);
...
export const initNewsDetail = () => {
  ...
  document.addEventListener("click", (e) => {
    const loadBtn = e.target.closest("#btnLoadMore");
    const closeBtn = e.target.closest("#btnCloseMore");
                    ...
                    <div class="max-w-6xl mx-auto px-6 md:px-10 animate-fade-in pb-20">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start mb-20">
                            ...
                            <div class="lg:col-span-5">
                                <div class="bg-slate-950 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl space-y-8">
                                    <h2 class="text-white font-black text-[10px] uppercase tracking-[0.4em] italic flex items-center gap-3">
                                    ...
                                    <div class="space-y-3 font-mono">
                                        <div class="flex justify-between items-center bg-slate-900/40 p-4 border border-slate-800/40 rounded-2xl">
                                            <span class="text-slate-600 text-[8px] uppercase font-bold">Author</span>
                                            <span class="text-white text-[10px] font-bold">${data.author || "UNKNOWN"}</span>
                                        </div>
                                        <div class="flex justify-between items-center bg-slate-900/40 p-4 border border-slate-800/40 rounded-2xl">
                                            <span class="text-slate-600 text-[8px] uppercase font-bold">Category</span>
                                            <span class="text-sky-500 text-[10px] font-bold uppercase">${data.category}</span>
                                        </div>
                                        <div class="flex justify-between items-center bg-slate-900/40 p-4 border border-slate-800/40 rounded-2xl">
                                            <span class="text-slate-600 text-[8px] uppercase font-bold">Date Upload</span>
                                            <span class="text-white text-[10px] font-bold">${data.date || "N/A"}</span>
                                        </div>
                                        <div class="flex justify-between items-center bg-sky-500/5 p-4 border border-sky-500/20 rounded-2xl">
                                            <span class="text-sky-500 text-[8px] font-black uppercase">Runtime Est</span>
                                            <span class="text-white text-[10px] font-bold italic uppercase">${data.readTime}</span>
                                        </div>
                                        <div class="flex justify-between items-center bg-emerald-500/5 p-4 border border-emerald-500/20 rounded-2xl">
                                            <span class="text-emerald-500 text-[8px] font-black uppercase">Language</span>
                                            <span class="text-white text-[10px] font-bold italic">${data.language || "STABLE"}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                                <div class="lg:col-span-8 space-y-12">
                                    <div class="prose prose-invert max-w-none">
                                        ...
                                        <p class="text-slate-400 text-lg md:text-xl leading-relaxed italic font-light mt-10">
                                            ${data.content}
                                        </p>
                                    </div>
                                    ...
                                    <div class="bg-slate-950 rounded-[2.5rem] border border-slate-800 overflow-hidden shadow-2xl relative">
                                        <div class="bg-slate-900/50 px-8 py-4 border-b border-slate-800 flex justify-between items-center">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <span class="text-[9px] font-mono text-slate-500 tracking-[0.3em] uppercase italic">${data.fileName || "logic.js"}</span>
                                            </div>
                                            <span class="text-[9px] font-mono text-sky-500 font-bold uppercase tracking-widest">${data.language || "Code"}</span>
                                        </div>
                                        <div class="p-10 font-mono text-xs md:text-sm leading-loose overflow-x-auto text-sky-400/80">
                                            <pre><code>${data.codeQuote || "// No snippet available"}</code></pre>
                                        </div>
                                    </div>

                                    <div class="pt-10 border-t border-slate-800/30">
                                        <button onclick="closeNewsDetail()" class="group flex items-center gap-4 bg-slate-900 hover:bg-sky-500/10 border border-slate-800 hover:border-sky-500/50 px-8 py-4 rounded-2xl transition-all duration-300">
                                            ...
                                            <span class="text-slate-500 group-hover:text-white font-mono text-[10px] uppercase tracking-[0.3em] italic">Back</span>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="lg:col-span-4 space-y-8">
                                    <div class="p-8 bg-sky-500/5 border border-sky-500/10 rounded-[2.5rem] space-y-5 relative overflow-hidden group">
                                        ...
                                        <p class="text-slate-400 text-sm leading-relaxed italic font-mono tracking-tighter uppercase">
                                            "${data.motivation || "Stay focused, stay humble."}"
                                        </p>
                                    </div>

                                    <div class="p-8 border border-dashed border-slate-800 rounded-[2.5rem] flex justify-center gap-10 text-slate-600">
                                        <a href="https://github.com" target="_blank" class="hover:text-white transition-all transform hover:scale-110">
                                            <i data-lucide="github" class="w-6 h-6"></i>
                                        </a>
                                        <button onclick="shareNews('${data.title}')" class="hover:text-sky-500 transition-all transform hover:scale-110">
                                            <i data-lucide="share-2" class="w-6 h-6"></i>
                                        </button>
                                        <button onclick="copyToClipboard(window.location.href)" class="hover:text-emerald-500 transition-all transform hover:scale-110">
